...
async function send(web3, signer, addr, encoded) {
...
async function main() {
  ...
  const argsABI = web3.eth.abi.encodeParameters(['uint80', 'int256', 'uint256','uint256', 'uint80'], [10, 15, 1200, 1200, 10]);
	// const argsABI = '';

  /* console.log(`Args abi is ${argsABI}`);
	const createTransaction = await alice.signTransaction(
		{
				data: Bytecode + argsABI.slice(2),
				value: "0x00",
				gasPrice: "0x01",
				gas: "0x1600000",
		}
  ...
  // await send(web3, alice, contractAddr, setCallAbi);
